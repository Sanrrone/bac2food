...
- `--allow-spices` / `--no-allow-spices` *(default off)* — include the
  `Spices and Herbs` category, scaled to a 2 g serving against the 100 g
  basis everything else is compared on. No run in the paper enables it.
...
**Stage 6 — composition axes** (added over rounds 5-15). Stages 1-5 strip;
this one puts back, as a labelled chunk, the few things a source says that
genuinely change what a bacterium is offered. A canon's members are averaged,
so any distinction the name does not carry is a distinction the numbers lose:
`cod, dried, salted` and `cod, dried, unsalted` both resolved to "dried" and
merged, and every cut of beef sat in one 1,291-member canon beside beef liver.

Seventeen axes are read, each independently and each appended in a fixed order:
state, salt, sugar, fat, fortification, moisture, plant part, packing medium,
oil pressing grade, colour, organ, cut, flavour, maturity, trim, provenance and
cooking fat. Three of them carry a value rather than a flag — the fat level when
a source states it as a number ("cheese, edam, 17% fat"; a stated range is kept
whole), the packing medium when it is named ("tuna, in olive oil") and the fat a
food was fried in when the source names it ("potato chip, homemade, fried, in
rapeseed oil"; an unnamed "vegetable oil" names nothing and is struck).

Provenance — wild or farmed — shows what a context gate is for. Farmed Atlantic
salmon carries about twice the fat of wild, and `salmon, atlantic` was holding
eight wild members and ten farmed. But of the 460 heads with a "wild" row only
16 have a farmed one as well: the rest are capture fisheries and game, where
wild is the unmarked case and the label only splits the canon. So the label is
printed for the foods that come both ways and withheld everywhere else — and
"wild rice" is left alone entirely, being a different grain rather than a
provenance.

Every axis needs both guards or it does damage. **Whole chunk only**: "Apples,
raw, red delicious" is a cultivar, not a colour. **Context gate**: "round" and
"breast" are cuts only behind an animal, "refined" is a pressing grade only on
a fat, "Common bean, Kidney" is a kidney BEAN and "Egg, white" the albumen.
The whole-chunk test is the right default but not always enough - the cheese
ageing grade is rarely a chunk of its own ("cheddar, natural shredded sharp"),
so it matches a word and leans on its cheese gate instead.

Two curated tables have the last word, keyed on the canon the rules produce:
`_CANON_MERGES` (mechanical, generated by counting which spelling more canons
already use) and then `_CANON_OVERRIDES` (hand-written, for names no rule can
reach). Because they are keyed on the finished name, a new axis label would
silently orphan every entry for the foods it labels, so both are consulted a
second time with the labels lifted off - and that retry is restricted to
entries that merely RE-SPELL a name, never to one that swaps the food.

A table's VALUES need the same care as its keys, and are easier to miss: a value
is a finished name that nothing re-canonicalises, so an entry still spelling
`yoghurt, plain` puts back exactly the split the rule dropping ", plain" had
just closed. Both halves of both tables are swept whenever a naming rule
changes.
...
|---|---|
| `Carrots, sliced or crinkle cut, frozen, unprepared - Proximates - NF9913X5` | `carrot` |
| `Carrots, baby, raw` | `carrot, baby` |
| `Beans, navy, mature seeds, raw` | `bean, navy` |
| `Beans, Dry, Navy (0% moisture)` | `bean, navy, dried, 0% moisture basis` |
...
| `Grapefruit juice, white, bottled, unsweetened, ocean spray` | `grapefruit juice, white` |
...
| `Wild mango/African mango/bush mango, pulp, unripe, raw (colour of peel: olive green) [biofoodcomp]` | `wild mango/african mango/bush mango, flesh` |
| `Wheat flour, white (industrial), 9% protein, bleached, enriched` | `wheat flour, enriched` |
| `Milk, fluid, nonfat (fat free or skim)` | `milk, fluid, fat-free` (paren stripped, second chunk kept) |
| `Potato, white (industrial), 50% extraction` | `potato, white` |
| `Mushrooms, shiitake, raw` | `shiitake mushroom` (plural folded; the variety survives) |
| `Mushroom, 'Agaricus bisporus'` | `mushroom` (Latin codes are cultivar-flood) |
| `Cheese, cheddar, sharp, sliced` | `cheese, cheddar, mature` (the ageing grade is an axis) |
| `Cheese, brie` | `cheese, brie` |
| `Cream, sour` | `sour cream` |
| `Pasta [Phenol-Explorer]` | `pasta` (category back-filled to `Cereal Grains and Pasta`) |

Net effect on the ANALYTICAL food set — the sixteen national databases the
shipped `food_nutrients.tsv` covers, branded label products excluded:
**116 053 foods → 22 094 canonical foods** (53 983 of those foods carry a
distinct description; the rest are per-panel duplicates of one). Heads that the
sources flood with cultivars, breeds and preparations collapse hard: `rice`
holds 17 canons, `apple` 9, `mushroom` 37. Heads where the qualifier is
composition keep it: `beef` holds 146 and `pork` 117, one per cut, organ,
trim level, provenance and stated fat percentage.

(The older figure quoted here — 131 930 catalog entries → 25 552 canons —
counted the full FDC catalog including branded label products, which the
published export does not carry. It is not comparable to the number above and
...
## Validation — demonstration cohort, 8-bacterium panel, v14 differential top picks
...
**Baseline → Phase 9 default → Phase 9 + substrate expansion (demonstration cohort):**
