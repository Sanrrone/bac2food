...
Since 18 August 2026 the deposited `food_nutrients.tsv` DOES carry the NEVO-derived values:
RIVM granted written permission to redistribute the harmonized values, on condition that the
reference "NEVO-online version 2025/9.0, RIVM, Bilthoven, The Netherlands" is carried and that
the changes made to their data are described (they are, in Methods). What the deposit does not
carry is the NEVO release itself — RIVM asked that users be pointed at their download page
instead — so this script keeps two uses:

  * rebuilding the partition against a NEWER NEVO release than 2025/9.0, which the deposit
    freezes; and
  * reproducing the harmonization end to end from the original source, rather than trusting
    the deposited rows.
